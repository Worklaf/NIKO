...
const AUDIO_CACHE_NAME = 'niko-audio-v2';      // ДОЛЖНО совпадать с sw.js!

const STATIC_CACHE_NAME = 'niko-music-v2';     // ДОЛЖНО совпадать с sw.js!
...
async function getOfflineAvailableTracks(allTracks) {
  ...
  // Если не переданы треки, загружаем из IndexedDB
  if (!allTracks || !allTracks.length) {
    allTracks = await getTracksFromDB();
  }
  ...
  if (!cachedUrls || !cachedUrls.length) {
    console.warn('⚠️ No cached audio files found');
    return [];
  }
  ...
  const filtered = allTracks.filter(t => {
    ...
    // Проверяем что аудио закешировано
    const audioCached = cachedSet.has(t.audio) || normalizedCached.has(normalizeUrl(t.audio));
    if (!audioCached) return false;

    // Если есть картинка, проверяем что она тоже закеширована
    if (t.cover) {
      const coverCached = cachedSet.has(t.cover) || normalizedCached.has(normalizeUrl(t.cover));
      // Если картинка не закеширована, все равно показываем трек (картинка будет загружена из кеша sw.js)
    }

    return true;
  });

  console.log('📊 Filtered', allTracks.length, 'tracks to', filtered.length, 'with cached audio');
  return filtered;
...
console.log('📦 offline-cache.js v2 loaded');
