...
async function imageCacheStrategy(request) {
  ...
  try {
    ...
    // Return default placeholder for offline
    const placeholder = 'data:image/svg+xml,%3Csvg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 120 200"%3E%3Crect fill="%230b0b10" width="120" height="200"/%3E%3C/svg%3E';
...
self.addEventListener('push', (e) => {
  ...
  // Если есть трек - добавляем информацию

  if (data.trackId) {

    options.title = data.title || 'New Track';

    options.body = data.body || 'Check out the new track!';
  ...
  e.waitUntil(
    ...
    self.registration.showNotification(data.title || 'N1K∅', options)
